...
class SRDataset(Dataset):
    ...
    def __init__(self, data_folder, split, crop_size, scaling_factor, lr_img_type, hr_img_type, test_data_name=None):
        ...
        # If this is a training or validation dataset, then crop dimensions must be perfectly divisible by the scaling factor
        ...
        if self.split == 'train':
            ...
            with open(os.path.join(data_folder, 'val_images.json'), 'r') as j:
                self.images = json.load(j)
        else:
